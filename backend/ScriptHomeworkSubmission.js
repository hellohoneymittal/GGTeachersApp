...
document.getElementById("raiseHWsubject").addEventListener("change", function () {
...
document.getElementById("dynamic-lp-list").addEventListener("change", function(e){
...
document.getElementById("selectHWContainer").addEventListener("change", function(e){
...
function validateLPSelection(ct_sub){
  ...
  selected.forEach(el => {
    ...
    if(!work_map[workType+"_"+chapter+"_"+source+" #% "+category]) work_map[workType+"_"+chapter+"_"+source+" #% "+category] = [];
    
    result["rows"][source].push(workType+" : "+row_num);
    work_map[workType+"_"+chapter+"_"+source+" #% "+category].push(text)
  });
...
async function openGenerateHomeworkWindow(){
  ...
  if (outputData?.status && outputData.response) {
    ...
    if(outputData.response.infoMsg)
      console.log("Information from Eligible Subject Function:\n\n"+outputData.response.infoMsg);

    if (Object.keys(eligibleHWList).length == 0) {
      SHOW_INFO_POPUP("No class-subjects available for raising Homework/Class Test Syllabus today!")
      homePageClick();
    } 
    else 
    {
  ...
  {
    SHOW_ERROR_POPUP("Unable to fetch the class-subjects for teacher: " + selectedTeacher + "!!");
    return;
  }
}
...
function submitLP(in_flag = 2){
  ...
  if(in_flag === 2){
    const selectedRadio = document.querySelector('input[name="worktype"]:checked');
    ...
    SHOW_CONFIRMATION_POPUP("Are you sure to submit today's work!",sendTodaysWorkBackend)
  }
  else{
    ...
    openVerifyDetailsWindow(["Chapter","Source","Work"],header_arr,outGrid,() => SHOW_CONFIRMATION_POPUP("Are you sure to submit today's work!",sendTodaysWorkBackend),"selectHWContainer")
  }
}
    ...
    if (outputData.response == "ok")
      ...
      SHOW_ERROR_POPUP(
        "Unable to submit today's work for: " +
          selectedLessonPlan["class"] + " : " + selectedLessonPlan["subject"] + "!!\n\n" +
          outputData.response.split("ERR: ")[1],
      );
  } else
    SHOW_ERROR_POPUP(
      "Unable to submit today's work for: " + selectedLessonPlan["class"] + " : " + selectedLessonPlan["subject"] + "!!",
    );
...
function showLPWindow(){
  ...
  let comment_needed = Number(eligibleHWList[selectedHwClass][selectedHwSubject]["comment"])
  let custom_input_label = eligibleHWList[selectedHwClass][selectedHwSubject]["custom_label"]
  let book_question_enabled = (Object.keys(eligibleHWList[selectedHwClass][selectedHwSubject]["question_map"]).length > 0)
  let i
  let next_button = document.getElementById("nextButtonLP")
...
function createMainAccordion(inputId) {
  ...
  Object.entries(eligibleHWList[selectedHwClass][selectedHwSubject]["question_map"]).forEach(([chapterTitle, sources]) => {
  ...
  chapterHeader.onclick = () => {
    chapterContent.classList.toggle("show") ? chapterHeader.classList.add("active") : chapterHeader.classList.remove("active")
  };
  ...
  Object.entries(sources).forEach(([sourceTitle, types]) => {
    ...
    sourceHeader.onclick = () => {
      sourceContent.classList.toggle("show") ? sourceHeader.classList.add("active") : sourceHeader.classList.remove("active")
    };
    ...
    Object.entries(types).forEach(([typeTitle, totalQuestions]) => {
      ...
      typeHeader.onclick = () => {
        typeContent.classList.toggle("show") ? typeHeader.classList.add("active") : typeHeader.classList.remove("active")
      };
